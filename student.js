/* ════════════════════════════════════════════════════════════════════ *
 *  student.js  —  ملف الطالب الرئيسي (نسخة كاملة ومُصلحة)              *
 *  التعديل: إصلاح مشكلة اختفاء الرسوم عند تحديث سبورة المدرس           *
 * ════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [002]  إعداد عناصر Canvas                                            *
 * ════════════════════════════════════════════════════════════════════ */

const _SW = window.screen.width;
const _SH = window.screen.height;
const CW_DEFAULT = Math.max(_SW - 66 - 80, 400);
const CH_DEFAULT = Math.max(_SH - 38 - 38 - 58, 300);

const cT = document.createElement('canvas');
const xT = cT.getContext('2d');
cT.width  = CW_DEFAULT;
cT.height = CH_DEFAULT;
...
/**
 * makeCanvas — ينشئ عنصر <canvas> ويُدرجه في مكدّس الرسم
 */
function makeCanvas(id) {
    ...
    if (paneS) {
        const canvasStack = paneS.querySelector('.canvas-stack');
        if (canvasStack) {
            canvasStack.prepend(cv);
        } else {
...
/** initCtx — يملأ جميع الـ canvas باللون الأبيض عند البداية */
function initCtx() {
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [003]  تغيير حجم Canvas الخاص بالطالب                               *
 * ════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [004]  تحميل صورة في سبورة الطالب (مُعدّل)                          *
 *  ✅ الآن يحافظ على رسوم الطالب بشكل مثالي                             *
 * ════════════════════════════════════════════════════════════════════ */
function loadStudentImage(url, clearDraw = true, callback) {
    const img = new Image();
    img.onload = () => {
        // ✅ تعديل أبعاد canvas لتطابق الصورة تماماً (يمنع التمدد)
        let newW = img.width;
        let newH = img.height;
        const MAX = 2000;
        ...
        // فقط أعد الحجم إذا اختلف
        if (newW !== CW || newH !== CH) {
            resizeStudentCanvases(newW, newH);
        }

        // رسم الصورة على canvas الخلفية
        xSBG.fillStyle = '#fff';
        xSBG.fillRect(0, 0, CW, CH);
        xSBG.drawImage(img, 0, 0, CW, CH);

        // مسح رسوم الطالب فقط إذا طُلب ذلك
        if (clearDraw) {
            xS.clearRect(0, 0, CW, CH);
        }
        ...
        // ملاءمة تلقائية بعد تحميل الصورة
        setTimeout(() => fitId('S'), 80);
    };
    img.onerror = () => {
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [005]  تحميل صورة في سبورة المدرس (cT)                              *
 * ════════════════════════════════════════════════════════════════════ */
function loadTeacherImage(url, callback) {
    ...
    img.onerror = () => {
        console.error('فشل تحميل صورة المدرس:', url);
        if (callback) callback();
    };
    img.src = url;
}
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [006]  التحكم بالتكبير والملاءمة (Zoom / Fit)                        *
 * ════════════════════════════════════════════════════════════════════ */
const sc = { T: 1, S: 1 };
...
function applyScale(id, s) {
    ...
    if (wr) wr.style.overflow = 'auto';

    if (id === 'S' && wr) {
        requestAnimationFrame(() => { wr.scrollLeft = 0; });
    }
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [007]  إدارة التبويبات (Tabs)                                        *
 * ════════════════════════════════════════════════════════════════════ */
let activeTab = 'S';

window.goTab = function (t) {
    activeTab = 'S';
    setTimeout(() => applyScale('S', 1), 80);
    if (approved && myId) db.ref('boardView/' + myId).set('S');
};


/* ════════════════════════════════════════════════════════════════════ *
 *  [008]  الإشعارات المنبثقة (Toast)                                    *
 * ════════════════════════════════════════════════════════════════════ */
...
let teacherDataChannel = null;
let teacherPC          = null;

let hlSnap       = null;
let examMode     = false;
let currentSlide = 'main';
let slideStrokes = {}; // تخزين الجروحات لكل شريحة

let teacherDrawing      = false;
let teacherDrawingTimer = null;

function markTeacherDrawing() {
    teacherDrawing = true;
    clearTimeout(teacherDrawingTimer);
    teacherDrawingTimer = setTimeout(() => { teacherDrawing = false; }, 800);
}
...
function markSubmitting() {
    ...
    submittingTimer = setTimeout(() => { submitting = false; }, 3000);
}

let lastCorrectedTs = 0;


/* ════════════════════════════════════════════════════════════════════ *
 *  [010]  تسجيل دخول الطالب                                             *
 * ════════════════════════════════════════════════════════════════════ */
...
window.quickEntry = function () {
    ...
    if (!nameEl.value.trim() && deviceName) nameEl.value = deviceName;
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [012]  الاستماع لأوامر المدرس عبر Firebase (مُصلح)                  *
 * ════════════════════════════════════════════════════════════════════ */
function listen() {
    ...
    db.ref('draw_t').on('child_added', snap => {
        ...
        if (d.shape) {
            if (activeStuSlide === 'main' && d.start && d.end) {
                const _s = { x: d.start.x * CW, y: d.start.y * CH };
                const _e = { x: d.end.x   * CW, y: d.end.y   * CH };
                drawShape(xSBG, d.shape, _s, _e, d.c, d.s || 4);
            }
            return;
        }
        if (!d.pts) return;
    ...
    /* ─── تعديل: معالجة تحميل صورة المدرس دون مسح كتابة الطالب ─── */
    db.ref('boardImg').on('value', snap => {
        ...
        // التحقق من شريحة الاختبار المحمية
        const currentSlObj = stuSlides.find(s => s.id === activeStuSlide);
        const isProtectedSlide = currentSlObj && (currentSlObj.isExam || currentSlObj.corrected);

        // إذا كان المدرس يرسم أو يتم رفع إجابة أو شريحة محمية، لا تفعل شيئاً
        if (teacherDrawing || submitting || isProtectedSlide) {
            return;
        }

        // التحقق من أننا نستهدف الشريحة الرئيسية حالياً
        if (activeStuSlide !== 'main') return;

        // تحميل الصورة كخلفية فقط (clearDraw = false) للحفاظ على طبقة الطالب
        loadStudentImage(d.data, false, () => {
            // إعادة رسم جروحات الطالب فوق الخلفية الجديدة
            if (slideStrokes['main'] && slideStrokes['main'].length > 0) {
                applyStrokesToCanvas(xS, slideStrokes['main']);
            }
            
            // تحديث الصورة المدمجة
    ...
    db.ref('correctedImg/' + myId).on('value', snap => {
        ...
        // ✅ استخدم slideId من المدرس إذا موجود — للربط بالشريحة الأصلية
        const corrId    = d.slideId || d.id || ('corr_' + Date.now());
        const corrLabel = d.slideLabel || d.label || 'تصحيح';

        // ✅ ابحث عن شريحة موجودة بنفس الـ ID وحدّثها بدل إنشاء جديدة
        const existing = stuSlides.find(s => s.id === corrId);
        if (existing) {
            existing.boardImg = d.data;
            existing.label    = corrLabel;
        } else {
            stuSlides.push({ id: corrId, label: corrLabel, isExam: false, corrected: true, boardImg: d.data });
        }

        if (approved && myId) {
            db.ref('studentExamSlides/' + myId + '/' + corrId).set({
                studentId: myId, studentName: myName,
                id: corrId, label: corrLabel, data: d.data, ts: Date.now()
            });
        }

        activeStuSlide = corrId;
        loadStudentImage(d.data, true, () => {
            lastMyBoardImg = mergedSnapshot();
            renderStuSlides();
            // ✅ fitId لضمان عرض الشريحة بشكل صحيح بدون تشويه أو تصغير
            setTimeout(() => fitId('S'), 120);
    ...
    db.ref('boardCmd').on('child_added', snap => {
        ...
        if (cmd.type === 'clear') {
            xT.fillStyle = '#fff';
            xT.fillRect(0, 0, cT.width, cT.height);
            tLP = null; tLS = null;
        }
        ...
        if (cmd.type === 'exam_question' && cmd.data) {
            ...
            loadStudentImage(cmd.data, true, () => {
                ...
                renderStuSlides();
                // ✅ fitId بدل applyScale(1) لضمان ملاءمة صحيحة بدون تشويه
                setTimeout(() => fitId('S'), 120);
            });
        }
...
function drawShape(ctx, t, s, e, c, lw) {
    ...
    if (t === 'line') {
        ...
        ctx.ellipse(
            (s.x + e.x) / 2, (s.y + e.y) / 2,
            Math.abs(e.x - s.x) / 2, Math.abs(e.y - s.y) / 2,
            0, 0, Math.PI * 2
        );
        ctx.stroke();
...
window.submitAnswer = function () {
    ...
    db.ref('studentAnswers/' + myId).set({
        ...
        toast('✅ تم رفع الإجابة بنجاح', 'g');
    }).catch(err => {
        toast('❌ فشل الرفع: ' + err.message, 'a');
    });
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [014]  استقبال رسومات المدرس عبر WebRTC DataChannel                  *
 * ════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [015]  أدوات رسم الطالب (قلم / ممحاة / تظليل)                       *
 * ════════════════════════════════════════════════════════════════════ */
...
function moveD(e) {
    ...
    if (tool === 'highlight' && hlSnap) {
        ...
        img.onload = () => {
            ...
            xS.beginPath();
            pts.forEach((pt, i) =>
                i ? xS.lineTo(pt.x * CW, pt.y * CH) : xS.moveTo(pt.x * CW, pt.y * CH)
            );
            xS.lineTo(p.x, p.y);
            xS.stroke();
            xS.globalAlpha = 1;
...
function flush() {
    ...
    // ✅ تأكد من تخزين الجروحة محلياً لإعادة رسمها لاحقاً
...
window.uploadHW = function (input) {
    ...
    r.onload = e => {
        ...
        img.onload = () => {
            ...
            lastMyBoardImg = mergedSnapshot();
            db.ref('boardImg_stu/' + myId).set({ data: lastMyBoardImg, ts: Date.now() });
            db.ref('studentImages/' + myId).set({
                studentId: myId, studentName: myName,
                data: lastMyBoardImg, ts: Date.now()
            });
            goTab('S');
            setTimeout(() => applyScale('S', 1), 100);
        };
...
window.toggleHand = () => {
    ...
    handRaised
        ? db.ref('handRaised/' + myId).set(true)
        : db.ref('handRaised/' + myId).remove();
};


/* ════════════════════════════════════════════════════════════════════ *
 *  [016]  استقبال صوت المدرس عبر WebRTC                                 *
 * ════════════════════════════════════════════════════════════════════ */
const ICE_SERVERS = [
...
function connectAudio() {
    db.ref('rtc/t2s_offer/' + myId).on('value', async snap => {
        ...
        db.ref('rtc/t2s_ice/' + myId).on('child_added', async s2 => {
            const ice = s2.val();
            if (ice) {
                try { await tPc?.addIceCandidate(new RTCIceCandidate(ice)); } catch (_) { }
                s2.ref.remove();
            }
        });
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [017]  إرسال صوت الطالب عبر WebRTC (ميكروفون)                       *
 * ════════════════════════════════════════════════════════════════════ */
...
async function startMicRTC() {
    ...
    db.ref('rtc/t2s_stu_ice/' + myId).on('child_added', async s2 => {
        const ice = s2.val();
        if (ice) {
            try { await sPc?.addIceCandidate(new RTCIceCandidate(ice)); } catch (_) { }
            s2.ref.remove();
        }
    });
...
window.toggleMic = async () => {
    ...
    if (!micOn) {
        try {
            ...
            db.ref('micRequests/' + myId).set({
                studentId: myId, studentName: myName, ts: Date.now()
            });
        } catch (err) {
...
/* ════════════════════════════════════════════════════════════════════ *
 *  [018]  قائمة الشرائح الجانبية (خاصة بكل طالب)                       *
 * ════════════════════════════════════════════════════════════════════ */
let stuSlides      = [];
...
function addExamSlide(boardImgData) {
    ...
    const id      = 'exam_' + myId + '_' + Date.now();
...
function renderStuSlides() {
    ...
    if (!list) return;
    list.innerHTML = '';
    stuSlides.forEach(sl => {
        const item = document.createElement('div');
        item.className  = 'stu-slide-item'
            + (sl.id === activeStuSlide ? ' active' : '')
            + (sl.isExam ? ' exam-slide' : '');
        item.dataset.id = sl.id;

        if (sl.boardImg) {
            item.innerHTML =
                '<img class="stu-slide-thumb" src="' + sl.boardImg + '">'
                + '<div class="stu-slide-label">' + sl.label + '</div>'
                + (sl.isExam ? '<div class="stu-slide-badge">✏️</div>' : '');
        } else {
            const _icon = sl.isExam ? 'pen' : 'chalkboard-teacher';
            item.innerHTML =
                '<div class="stu-slide-empty"><i class="fas fa-' + _icon + '"></i></div>'
                + '<div class="stu-slide-label">' + sl.label + '</div>';
        }
...
function switchStuSlide(slideId) {
    ...
    renderStuSlides();
    // ✅ استخدم fitId بدل applyScale(1) حتى تملأ الشريحة المساحة بشكل صحيح
    setTimeout(() => fitId('S'), 120);
}


/* ════════════════════════════════════════════════════════════════════ *
 *  [019]  قياس زمن الاستجابة (Ping / Latency)                           *
 * ════════════════════════════════════════════════════════════════════ */
setInterval(() => {
    const t = Date.now();
    db.ref('ping_s').set(t);
    setTimeout(() => {
        db.ref('ping_s').once('value', () => {
